...
describe("web processMessage inbound context", () => {
  ...
  it("injects WhatsApp group prompts into GroupSystemPrompt for group chats", async () => {
    await processMessage(
      makeProcessMessageArgs({
        ...
        cfg: {
          channels: {
            whatsapp: {
              groupSystemPrompt: "Group default prompt",
              groups: {
    ...
    expect(capturedCtx).toBeTruthy();
    // oxlint-disable-next-line typescript/no-explicit-any
    expect((capturedCtx as any).GroupSystemPrompt).toBe(
      "Group default prompt\n\nSpecific group prompt",
    );
  });
    ...
    await processMessage(
      makeProcessMessageArgs({
        ...
        cfg: {
          channels: {
            whatsapp: {
              directSystemPrompt: "Direct default prompt",
              direct: {
